...
global.document = {
  ...
  querySelectorAll(sel){ return sel.includes('.rchart') ? charts.map(k=>({dataset:{r:k}, appendChild(c){ if(c && c.tag==='svg') __svgs.push(c); }})) : []; },
...
process.stdout.write(__svgs.map(renderEl).join("\n"));
