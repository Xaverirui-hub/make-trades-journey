/* ---------------------------------------------------------------------------
   Source     React Bits — <GradientWaves />   https://reactbits.dev
   ...
   --------------------------------------------------------------------------- */
/* ============================================================================
   GradientWaves — MTJ theme port
   ----------------------------------------------------------------------------
   Vanilla ES module port of the React Bits <GradientWaves /> component.
   ...
   ~8 KB from our own origin.

   WebGL2 is required, not preferred: the shader is `#version 300 es`.
   It premultiplies (`fragColor = vec4(col * alpha, alpha)`), so the context
   uses premultipliedAlpha and blends ONE / ONE_MINUS_SRC_ALPHA.

   This is a raymarcher - up to 128 steps per pixel - so the pixel ratio is
   capped tighter than the others and the loop parks when off-screen.

   Theme, by structure rather than by tuning: horizonColor is the far haze and
   sits where alpha is lowest, waveColor and crestColor are the near wave
   bodies and crests where alpha is highest. Violet on the horizon therefore
   stays a suggestion in the distance while gold owns everything the eye
   actually lands on.
   ========================================================================== */
...
const DEFAULTS = {
  horizonColor: '#4B2E8F',  /* far haze - the only place violet lives */
  waveColor: '#C9A227',     /* wave bodies - MTJ gold-deep */
  crestColor: '#FCE9A8',    /* nearest crests - gold-bright */
  speed: 0.35,
  amplitude: 2.5,
  waveScale: 0.6,
  waveRatio: 0.9,
  ...
  maxDpr: 1.3   /* raymarcher: keep the pixel count honest */
};
...
/**
 * Mount the wave field into `target` (selector or element).
 * Adds `.is-live` once the first frame has drawn.
 * Returns a teardown function, or null if nothing mounted.
 */
export function mountGradientWaves(target, options = {}) {
  ...
  gl.uniform1f(U.uWaveScale, o.waveScale);
  gl.uniform1f(U.uWaveRatio, o.waveRatio);
  gl.uniform1f(U.uSwell, o.swell);
  gl.uniform1f(U.uTurbulence, o.turbulence);
  gl.uniform1f(U.uTilt, o.tilt);
  gl.uniform1f(U.uZoom, o.zoom);
  gl.uniform1f(U.uHeight, o.height);
  gl.uniform1f(U.uFogDepth, o.fogDepth);
  gl.uniform1f(U.uSteps, STEPS[o.detail] ?? 70);
  ...
  gl.uniform1f(U.uParallax, o.parallaxStrength);
  gl.uniform1i(U.uEnableMouse, o.mouseInteraction ? 1 : 0);
  gl.uniform3fv(U.uHorizonColor, hexToRgb(o.horizonColor));
  gl.uniform3fv(U.uWaveColor, hexToRgb(o.waveColor));
  gl.uniform3fv(U.uCrestColor, hexToRgb(o.crestColor));
...
export default mountGradientWaves;
