...
function renderEl(e){
  let a = Object.entries(e.attrs||{}).map(([k,v])=>` ${k}="${String(v).replace(/&/g,'&amp;').replace(/"/g,'&quot;')}"`).join('');
  let inner = (e._t!==undefined && e._t!==null) ? e._t.replace(/&/g,'&amp;').replace(/</g,'&lt;') : e.kids.map(renderEl).join('');
  return `<${e.tag}${a}>${inner}</${e.tag}>`;
}
const charts = ['rsiLevels','rsiFormula','rsiDivergence','rsiTrend','rsi50','rsiPractice','rsiMisuse'];
global.document = {
  createElementNS(ns, tag){ return node(tag, {}); },
  querySelectorAll(sel){ return sel.includes('.rchart') ? charts.map(k=>({dataset:{r:k}, appendChild(c){out += renderEl(c);}})) : []; },
  getElementById(){ return node('x',{}); },
  createElement(){ return node('x',{}); },
